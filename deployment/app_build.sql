...
------------------------------------------------------------------------------
--Rebuild app
-- Work in the same schema that owns the stage

USE ROLE ACCOUNTADMIN;
USE DATABASE GOV_APP;
USE SCHEMA GOV_APP.APP;
...
CREATE OR REPLACE STREAMLIT GOVERNANCE_APP
  ROOT_LOCATION = '@APP_STAGE'      -- points to where Home.py & lib/ live
  MAIN_FILE     = 'Home.py'         -- matches your screenshot
  QUERY_WAREHOUSE = GOV_WH
  TITLE = 'Governance Platform';

-- grants so your users can open it
GRANT USAGE ON STREAMLIT GOV_APP.APP.GOVERNANCE_APP TO ROLE GOVERNANCE_ADMIN;
GRANT USAGE ON WAREHOUSE GOV_WH TO ROLE GOVERNANCE_ADMIN;
