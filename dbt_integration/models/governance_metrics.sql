-- ====================================================================
-- DBT MODEL: GOVERNANCE METRICS (aligned to GOV_PLATFORM tables)
-- ====================================================================
{{ config(
    materialized='table',
...
WITH daily_process_metrics AS (
    SELECT
        ...
        '__ALL__'          AS domain_name,  -- process runs are not tied to domains
        COUNT(*) AS total_runs,
    ...
    FROM {{ source('gov_platform', 'process_run') }} pr
...
daily_quality_metrics AS (
    SELECT
        DATE(res.created_at) AS metric_date,
        d.domain_name,
        COUNT(*) AS total_dq_tests,
        COUNT(CASE WHEN res.outcome = 'PASS' THEN 1 END) AS passed_tests,
        COUNT(CASE WHEN res.outcome = 'FAIL' THEN 1 END) AS failed_tests,
        COUNT(CASE WHEN res.outcome = 'WARN' THEN 1 END) AS warning_tests
    FROM {{ source('gov_platform', 'dq_result') }} res
    LEFT JOIN {{ source('gov_platform', 'dim_dataset') }} ds ON ds.dataset_id = res.dataset_id
    LEFT JOIN {{ source('gov_platform', 'dim_domain') }} d ON d.domain_id = ds.domain_id
    WHERE res.created_at >= DATEADD('day', -30, CURRENT_DATE())
    GROUP BY 1, 2
),
...
daily_control_metrics AS (
    ...
    LEFT JOIN {{ source('gov_platform', 'dim_dataset') }} ds 
           ON ds.dataset_id = cr.scope_id AND cr.scope_type = 'DATASET'
    LEFT JOIN {{ source('gov_platform', 'dim_domain') }} d ON d.domain_id = ds.domain_id
...
SELECT
    ...
    COALESCE(NULLIF(qm.domain_name, ''), NULLIF(cm.domain_name, ''), pm.domain_name) AS domain_name,
    ...
    CASE WHEN pm.total_runs > 0 THEN (pm.successful_runs * 100.0 / pm.total_runs) ELSE NULL END AS process_success_rate_pct,
    ...
    CASE WHEN qm.total_dq_tests > 0 THEN (qm.passed_tests * 100.0 / qm.total_dq_tests) ELSE NULL END AS dq_pass_rate_pct,
    ...
    CASE WHEN cm.total_control_tests > 0 THEN (cm.passed_controls * 100.0 / cm.total_control_tests) ELSE NULL END AS control_pass_rate_pct,

    -- Simple overall health
    CASE 
        WHEN COALESCE(pm.total_runs, 0) + COALESCE(qm.total_dq_tests, 0) + COALESCE(cm.total_control_tests, 0) > 0 THEN
            (COALESCE(pm.successful_runs, 0) * 40 + COALESCE(qm.passed_tests, 0) * 35 + COALESCE(cm.passed_controls, 0) * 25) * 100.0 / 
            NULLIF((COALESCE(pm.total_runs, 0) * 40 + COALESCE(qm.total_dq_tests, 0) * 35 + COALESCE(cm.total_control_tests, 0) * 25), 0)
        ELSE NULL
